        ...
        try {
            ...
            if (_aeNewText && _aeChatId && _aeOrigId) {
                ...
                if (!_aeOwnerJid || !_aeOrigMsg || _aeOrigMsg.key?.fromMe || _aeSenderNum === _aeBotNum || _aeEditedByNum === _aeBotNum) {
                    ...
                    // default 'private' → forward to owner DM
            ...
            const _adOwnerJid = getBotJid(devtrust);
            {
                ...
                if (_adOriginal) {
                    ...
                    let _adText = `*🔰 ANTIDELETE REPORT 🔰*\n\n` +
                        ...
                        `*👤 Sender:* @${_adSenderNum}\n` +
                        `*🕒 Time:* ${_adTime}\n` +
                        (_adIsGroup ? `*👥 Chat:* Group\n` : `*💬 Chat:* Private\n`);
                    if (_adGroupName) _adText += `*👥 Group:* ${_adGroupName}\n`;
    ...
    if (getSetting(botNumber, "autoStatusReply", false)) {
        ...
        if (_statusReplyMsg && m.key.participant) {
            try {
                ...
                console.log("❌ Error replying to status:", err);
            }
        }
    }
}
...
♱  ${greeting}, *${pushname}*
*CYBER* ᴀᴛ ʏᴏᴜʀ sᴇʀᴠɪᴄᴇ
📱 *Pair CYBER:* _https://t.me/cybersecpro7_bot
❖═━═══𖠁𐂃𖠁══━═❖
...
♱  ${greeting}, *${pushname}*
*CYBER* ᴀᴛ ʏᴏᴜʀ sᴇʀᴠɪᴄᴇ
📱 *Pair:* _https://t.me/cybersecpro7_bot
❖═━═══𖠁𐂃𖠁══━═❖
...
case "repo": {
    ...
    let caption = `📂 *CYBER Repository*\n\n` +
        `🤖 Bot 1: https://t.me/cybersecpro7_bot\n` +
        `🤖 Bot 2: https://t.me/CYBERmdbot\n\n` +
        `📢 Updates:\n${tgChannel}\n${waChannel}`;
...
case 'ae': {
    ...
    const _aeStatusLabel = _aeCurrentMode === 'private' ? '🔒 PRIVATE (owner DM only)' :
        (_aeCurrentMode === 'chat' || _aeCurrentMode === 'true') ? '💬 CHAT (same group/chat)' : '❌ OFF';
    if (!_aeOption) {
        return reply(
            `*✏️ ANTI-EDIT SETTINGS*\n\n` +
            `*Current Status:* ${_aeStatusLabel}\n\n` +
            `*Commands:*\n` +
            `• \`${prefix}antiedit on\` — Enable (report in same chat)\n` +
            `• \`${prefix}antiedit private\` — Forward edits to owner only\n` +
            `• \`${prefix}antiedit off\` — Disable\n\n` +
            `*Modes:*\n` +
            `📌 *chat* — Show edited message in same chat\n` +
            `🔒 *private* — Send only to owner's inbox`
        );
    }
...
case 'adel': {
    ...
    if (!_adAction) {
        return reply(
            `*🔰 ANTIDELETE SETTINGS 🔰*\n\n` +
            `*Current Status:* ${_adCfgNow.enabled ? '✅ Enabled' : '❌ Disabled'}\n\n` +
            `*Commands:*\n` +
            `• \`${prefix}antidelete on\` — Enable\n` +
            `• \`${prefix}antidelete off\` — Disable\n\n` +
            `*Features:*\n` +
            `• Track deleted messages\n` +
            `• Save deleted media (image/video/audio/sticker)\n` +
            `• Send deletion reports to owner's DM`
        );
    }
    if (_adAction === 'on') {
        ...
        return reply(
            ...
            `• Send deletion reports to owner DM`
        );
    }
    if (_adAction === 'off') {
...
case 'ig': {
    ...
    try {
        ...
        if (igVideoUrl) {
            ...
            await devtrust.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
            reply("❌ *No downloadable media found*");
        }
    } catch (err) {
